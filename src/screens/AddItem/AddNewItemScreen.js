...
export default function AddNewItemScreen({ navigation }) {
  ...
  const pickImage = async () => {
    ...
    if (!result.canceled && result.assets && result.assets.length > 0) {
      setImage(result.assets[0].uri);  // Set image URI correctly from assets array
  ...
  const handleConfirm = async () => {
    ...
    try {
        ...
        if (image) {
            ...
            const blob = await response.blob(); // Create the blob from the image URI
            const imageName = `item_${user.uid}_${Date.now()}.jpg`; // Create a unique name for the image
            const imageRef = ref(storage, `items/${user.uid}/${imageName}`); // Use the unique name in the reference
